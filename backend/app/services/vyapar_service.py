...
async def _confirm_upi_booking(
    ...
    # 2. Lookup booking directly from Supabase first
    booking = None
    if db_manager.is_connected and target_b_id:
        try:
            row = await db_manager.fetch_one(
            ...
            logger.warning(f"Error fetching booking by ID: {e}")

    # Fallback to in-memory store
    if not booking and target_b_id:
        booking = BOOKINGS_STORE.get(target_b_id)

    # Fallback to the latest PENDING booking if ID did not resolve
    if not booking and db_manager.is_connected:
        try:
            logger.warning("No booking found by ID; matching against latest PENDING booking.")
            row = await db_manager.fetch_one(
                "SELECT * FROM bookings WHERE booking_status = 'PENDING' ORDER BY created_at DESC LIMIT 1"
            )
            if row:
                booking = dict(row)
                target_b_id = booking["booking_id"]
        except Exception as e:
            logger.error(f"Fallback pending lookup failed: {e}")

    if not booking:
        raise HTTPException(status_code=404, detail="Booking details not found to confirm payment.")

    actual_booking_id = booking.get("booking_id", target_b_id)
    ...
    # 3. Transition seats to permanent BOOKED status
    await SeatLockService.permanently_book_seats(
        show_id=show_id,
    ...
    # Direct database updates for seats and bookings
    if db_manager.is_connected:
        try:
            if seat_ids:
                await db_manager.execute(
            ...
            await db_manager.execute(
                ...
                WHERE booking_id = $3
                """,
                payment_id, utr_number, actual_booking_id
            )
            ...
            await db_manager.execute(
                ...
                ON CONFLICT DO NOTHING
                """,
                order_id, payment_id, actual_booking_id, float(booking.get("total_amount", 1.0))
            )
            logger.info(f"Supabase booking {actual_booking_id} successfully confirmed with UTR {utr_number}")
        except Exception as e:
            logger.error(f"Supabase confirmation update error: {e}")
...
async def check_upi_status(order_id: str):
    ...
    # Check database directly for confirmation
    ...
    # Live API status check with VyaparGateway
    if not is_paid:
        try:
            ...
            logger.debug(f"Vyapar live poll notice: {check_err}")
...
async def receive_vyapar_webhook(request: Request):
    """
    Webhook listener for VyaparGateway with signature validation and direct database updates.
    """
    try:
    ...
    client_txn_id = payload.get("client_txn_id") or data.get("client_txn_id") or ""
    order_id = payload.get("order_id") or data.get("order_id") or order_id_hdr or client_txn_id
    ...
    # Confirm booking and lock seats in Supabase
    await _confirm_upi_booking(
        order_id=target_order_id,
        payment_id=payment_id,
