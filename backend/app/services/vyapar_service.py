...
class VyaparService:
    ...
    def verify_webhook_signature(
        ...
        if not signature:
            logger.warning("No signature header received from gateway in webhook request.")
            return False
        ...
        try:
            ...
            if timestamp:
                ts_sign = f"{timestamp}.{raw_body_str}"
                ts_hash = hmac.new(
                    secret.encode("utf-8"),
                    ts_sign.encode("utf-8"),
                    hashlib.sha256
                ).hexdigest()
                if hmac.compare_digest(ts_hash.lower(), signature.lower()):
                    return True

            direct_hash = hmac.new(
                secret.encode("utf-8"),
                raw_body_bytes,
                hashlib.sha256
            ).hexdigest()
            if hmac.compare_digest(direct_hash.lower(), signature.lower()):
                return True

            logger.warning(
                f"Signature mismatch! Received: '{signature}', Computed: '{direct_hash}'"
            )
            return False
