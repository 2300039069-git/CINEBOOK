...
const TheatreShowtimesCard = ({
  ...
  return (
    <div className="p-5 sm:p-6 rounded-2xl bg-surface border border-border hover:border-primary/40 transition-all space-y-4 shadow-sm text-text-primary">
      ...
      <div className="flex items-center justify-between px-3 py-2 rounded-xl bg-surface-elevated border border-border text-xs">
      ...
      {amenities && amenities.length > 0 && (
        <div className="flex flex-wrap items-center gap-2">
          {amenities.map((item, idx) => (
            <span key={idx} className="px-2.5 py-0.5 rounded-lg bg-surface-elevated text-text-secondary border border-border text-[11px] font-semibold">
              {item}
            </span>
          ))}
        </div>
