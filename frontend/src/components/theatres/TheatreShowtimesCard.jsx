...
export const TheatreShowtimesCard = ({
  ...
  return (
    <div className="p-5 sm:p-6 rounded-3xl bg-surface border border-border hover:border-primary/40 transition-all space-y-4 shadow-sm text-text-primary">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 pb-3.5 border-b border-border">
        <div>
          <div className="flex items-center gap-2">
            ...
            <h3 className="text-base sm:text-lg font-black text-text-primary tracking-tight font-display">{theatreName}</h3>
          </div>
        ...
        <div className="sm:text-right">
          ...
          <span className="text-sm font-black text-primary font-mono">{priceRange}</span>
        </div>
      </div>

      {/* Active Day & Date Banner */}
      <div className="flex items-center justify-between px-3 py-2 rounded-2xl bg-surface-elevated border border-border text-xs">
        <div className="flex items-center gap-2 text-text-secondary">
          <Calendar className="w-3.5 h-3.5 text-primary" />
          <span className="font-bold text-text-primary">{formattedDate}</span>
        </div>
        <span className="text-[10px] font-black text-primary uppercase tracking-wider bg-primary/10 px-2 py-0.5 rounded-lg border border-primary/20">
          Laser Showtimes
        </span>
      </div>
      ...
      {amenities && amenities.length > 0 && (
        <div className="flex flex-wrap items-center gap-1.5">
          {amenities.map((item, idx) => (
            <span key={idx} className="px-2.5 py-0.5 rounded-xl bg-surface-elevated text-text-secondary border border-border text-[11px] font-semibold">
              {item}
            </span>
          ))}
        </div>
          ...
          {timeSlots.map((slot, idx) => {
            ...
            return (
              <button
                ...
                className={`relative py-2.5 px-3 rounded-2xl border text-xs font-bold transition-all text-center cursor-pointer ${
                  isSelected
              ...
              >
                <span>{slot}</span>
                {isFast && !isSelected && (
                  <span className="absolute -top-1.5 -right-1 px-1.5 py-0.2 rounded-full bg-accent text-black text-[8px] font-black uppercase shadow-xs">
                    Fast
                  </span>
                )}
              </button>
      ...
      <div className="pt-2">
        <button
          ...
          className="w-full py-3.5 rounded-2xl bg-primary hover:bg-primary-hover text-white text-xs sm:text-sm font-extrabold uppercase tracking-wider flex items-center justify-center gap-2 shadow-md shadow-primary/20 transition-all cursor-pointer active:scale-98"
        >
