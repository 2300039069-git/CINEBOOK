...
const ShowtimeFilter = ({
  ...
  return (
    <div className="space-y-6 text-slate-100">
      ...
      <div className="bg-[#11141D] py-3.5 px-4 rounded-xl border border-[#1E2332]">
            ...
            return (
              <button
                ...
                className={`flex flex-col items-center justify-center min-w-[70px] sm:min-w-[80px] py-2.5 px-2 rounded-xl border transition-all cursor-pointer ${
                  isSelected
                    ? 'bg-[#E50914] border-[#E50914] text-white font-bold shadow-sm'
                    : 'bg-[#181C28] border-[#1E2332] text-slate-300 hover:border-slate-500 hover:text-white'
                }`}
              >
                <span className={`text-[10px] font-bold tracking-wider ${isSelected ? 'text-white/90' : 'text-slate-400'}`}>
                  {item.dayName}
                </span>
                <span className="text-base sm:text-lg font-extrabold my-0.5">
                  {item.dayNumber}
                </span>
                <span className={`text-[10px] font-medium ${isSelected ? 'text-white/90' : 'text-slate-400'}`}>
                  {item.monthName}
                </span>
              </button>
            );
      ...
      <div className="flex flex-wrap items-center justify-between gap-4 p-4 bg-[#11141D] border border-[#1E2332] rounded-xl text-xs">
        {/* Quick Filter Chips */}
        <div className="flex flex-wrap items-center gap-2 sm:gap-4">
          ...
          <div className="h-4 w-[1px] bg-[#1E2332] hidden md:block" />
              ...
              <button
                ...
                className={`px-2.5 py-1 rounded-lg transition-all border cursor-pointer ${
                  selectedLanguage === lang
                    ? 'bg-[#181C28] border-[#F59E0B] text-[#F59E0B] font-bold shadow-sm'
                    : 'bg-[#181C28] border-[#1E2332] text-slate-300 hover:border-slate-500'
                }`}
              >
                {lang}
        ...
        <div className="flex items-center gap-4 text-[11px] font-semibold text-slate-400">
          ...
          <span className="flex items-center gap-1.5">
            <span className="w-2 h-2 rounded-full bg-[#F59E0B]"></span> Fast Filling
          </span>
          <span className="flex items-center gap-1.5">
            <span className="w-2 h-2 rounded-full bg-[#E50914]"></span> Almost Full
          </span>
        </div>
      </div>
      ...
      <div className="space-y-4">
        {filteredTheatres.length === 0 ? (
          <div className="p-12 text-center bg-[#11141D] border border-[#1E2332] rounded-xl">
          ...
          filteredTheatres.map((theatre) => (
            <div
              key={theatre.id}
              className="p-5 sm:p-6 bg-[#11141D] border border-[#1E2332] hover:border-slate-600 rounded-xl space-y-4 shadow-sm transition-all"
            >
              ...
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 pb-3 border-b border-[#1E2332]">
                ...
                <div className="flex items-center gap-2.5 text-xs">
                  <span className="flex items-center gap-1 text-emerald-400 font-medium bg-[#181C28] border border-[#1E2332] px-2.5 py-1 rounded-lg">
                    <Smartphone className="w-3.5 h-3.5" />
                    <span>M-Ticket</span>
                  </span>
                  <span className="flex items-center gap-1 text-[#F59E0B] font-medium bg-[#181C28] border border-[#1E2332] px-2.5 py-1 rounded-lg">
                    <Utensils className="w-3.5 h-3.5" />
                    <span>F&B Order</span>
                  </span>
                  <span className="text-[11px] text-slate-400 bg-[#181C28] border border-[#1E2332] px-2.5 py-1 rounded-lg">
                  ...
                  return (
                    <button
                      ...
                      className={`group relative flex flex-col items-center justify-center p-3 rounded-lg border transition-all min-w-[125px] text-center cursor-pointer ${
                        isAlmostFull
                          ? 'border-[#E50914]/50 bg-[#E50914]/10 hover:border-[#E50914]'
                          : isFillingFast
                          ? 'border-[#F59E0B]/50 bg-[#F59E0B]/10 hover:border-[#F59E0B]'
                          : 'bg-[#181C28] border-[#1E2332] hover:border-slate-500'
                      }`}
                    >
                      <span className="text-sm font-extrabold text-white group-hover:text-[#E50914] transition-colors">
                        {show.time}
                      </span>
                      ...
                      {/* Fast Tag */}
                      {isFillingFast && (
                        <span className="absolute -top-2 -right-1 px-1.5 py-0.2 rounded-full bg-[#F59E0B] text-black text-[8px] font-bold uppercase">
                          Fast
                        </span>
                      )}
                    </button>
