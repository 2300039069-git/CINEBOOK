...
const PrivateAccessGate = ({ children }) => {
  ...
  return (
    <div className="min-h-screen bg-[#080B10] text-[#F8FAFC] flex flex-col items-center justify-center p-4 relative overflow-hidden font-sans">
      {/* Background Ambient Glows */}
      <div className="absolute top-1/4 left-1/4 w-96 h-96 bg-[#E50914]/15 rounded-full blur-[140px] pointer-events-none" />
      <div className="absolute bottom-1/4 right-1/4 w-96 h-96 bg-[#D4AF37]/15 rounded-full blur-[140px] pointer-events-none" />

      <div className="max-w-md w-full relative z-10 p-8 rounded-3xl bg-[#0F1523]/90 border border-[#1E293B] backdrop-blur-2xl shadow-2xl space-y-6 text-center animate-fade-in">
        {/* Brand Icon */}
        ...
        <div className="space-y-1.5">
          <span className="text-[10px] font-black uppercase tracking-widest text-[#D4AF37] flex items-center justify-center gap-1">
        ...
        <form onSubmit={handleUnlock} className="space-y-4">
          <div className="relative">
            ...
            <input
              ...
              autoFocus
              className="w-full pl-11 pr-4 py-3.5 rounded-2xl bg-[#080B10] border border-[#1E293B] text-white placeholder:text-[#64748B] text-sm font-bold text-center tracking-widest focus:outline-none focus:border-[#D4AF37] transition-all"
            />
          </div>
          ...
          <button
            type="submit"
            className="w-full py-3.5 rounded-2xl bg-gradient-to-r from-[#D4AF37] to-[#E2B714] hover:from-[#E2B714] hover:to-[#D4AF37] text-black text-xs font-black uppercase tracking-wider shadow-glow-gold transition-all flex items-center justify-center gap-2 group cursor-pointer"
          >
        ...
        {/* Footer info */}
        <div className="pt-2 border-t border-[#1E293B] text-[11px] text-[#94A3B8] flex items-center justify-center gap-1">
          <ShieldCheck className="w-3.5 h-3.5 text-emerald-400" />
          <span>Exhibitor & Customer Flow Protected • Passcode: <strong className="text-white">2026</strong></span>
        </div>
      </div>
    </div>
  );
