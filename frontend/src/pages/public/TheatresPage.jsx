      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 space-y-8">
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 pb-6 border-b border-border">
          ...
          <div className="flex items-center gap-3">
            ...
            <button
              ...
              className="px-4 py-2.5 rounded-2xl bg-surface hover:bg-surface-elevated border border-border text-text-primary text-xs font-bold whitespace-nowrap cursor-pointer transition-colors shadow-xs"
            >
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          {cityTheatres.map((theatre) => (
            <div
              key={theatre.id}
              className="p-6 sm:p-7 rounded-3xl bg-surface border border-border hover:border-primary/40 transition-all space-y-5 shadow-sm group"
            >
              <div className="flex items-start justify-between gap-4">
                <div className="flex items-start gap-3.5">
                  ...
                  <div>
                    <h3 className="font-black text-base sm:text-lg text-text-primary group-hover:text-primary transition-colors font-display">
                      {theatre.name}
                    </h3>
                ...
                <span className="px-3 py-1 rounded-xl bg-surface-elevated border border-border text-[11px] text-accent font-black flex-shrink-0">
              ...
              {/* Facilities */}
              <div>
                <span className="text-[10px] font-black uppercase text-text-muted block mb-2 tracking-wider">
                ...
                <div className="flex flex-wrap gap-1.5">
                  {(theatre.facilities || ['4K RGB Laser', 'Dolby Atmos 64-Channel', 'Pushback Plush Seating']).map((fac) => (
                    <span
                      key={fac}
                      className="px-2.5 py-1 rounded-xl bg-surface-elevated text-text-secondary border border-border text-xs font-semibold"
                    >
                      {fac}
                    </span>
                  ))}
              ...
              <div className="pt-2 border-t border-border">
                <span className="text-[10px] font-black uppercase text-primary block mb-2 tracking-wider flex items-center gap-1">
                ...
                <div className="grid grid-cols-2 sm:grid-cols-4 gap-2">
                  {['11:00 AM', '02:30 PM', '06:15 PM', '09:45 PM'].map((slot) => (
                    <button
                      ...
                      className="py-2.5 px-3 rounded-2xl bg-surface-elevated border border-border hover:border-primary hover:bg-primary/10 hover:text-primary text-xs font-bold text-text-primary transition-all text-center cursor-pointer shadow-xs"
                    >
                      {slot}
                    </button>
                  ))}
              ...
              <div className="pt-3 border-t border-border flex items-center justify-between">
                ...
                <Link
                  to="/movies"
                  className="flex items-center gap-1.5 px-4 py-2 rounded-2xl bg-primary hover:bg-primary-hover text-white text-xs font-bold shadow-md transition-all cursor-pointer"
                >
